...
oadm registry --create --credentials="${MASTER_CONFIG_DIR}/openshift-registry.kubeconfig" --config="${ADMIN_KUBECONFIG}" --images="${USE_IMAGES}"
...
oc create -n openshift -f examples/image-streams-contos7.json --config="${ADMIN_KUBECONFIG}"
...
KUBECONFIG="${ADMIN_KUBECONFIG}" \
	...
	go test -v -tags=ldap ./test/extended
